...
} from "@/redux/features/feathers/blogs/blogApi";
...
const DashboardBlogPage = () => {
  ...
  if (isLoading) {
    return (
      <div className="pl-[43%] text-3xl font-medium text-primary mt-12">
        Loading...
      </div>
    );
  }
      ...
      <div className=" md:w-4/5 mx-auto flex flex-col shadow-2xl rounded-lg mt-5 mb-10 p-8 pt-2">
        <div className="flex justify-between items-center mb-2">
          ...
          <Link href="/dashboard/blogs/add-blog">
            <button className="btn hover:border-primary py-2 text-white">
              Add Blog
            </button>
          </Link>
        </div>
        <table className="w-auto">
          <thead className="bg-gray-200">
            <tr>
              <th className="w-0 text- px-3 py-2 border border-slate-300">
                Serial
              </th>
              ...
              <th className="text- px-6 py-3 border border-slate-300">
                Delete
              </th>
            </tr>
          </thead>
          <tbody>
            {data?.map((item: TSkills, index: number) => (
              <tr key={index}>
                <td className="text-center text-lg font-medium border border-slate-300">
                  {index + 1}
                </td>
                <td className="text-xl font-medium text-center border border-slate-300">
                  {item?.name}{" "}
                </td>
                ...
                <td className="px-6 py-3 w-1 mx-auto text-lg border border-slate-300">
                  <MdDelete
                    ...
                    className="size-10 p-2 rounded-full text-red-500 bg-gray-200 cursor-pointer"
                  />
                </td>
              </tr>
            ))}
